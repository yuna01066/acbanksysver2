...
const tempDir = await mkdtemp(path.join(tmpdir(), 'acbank-tax-invoice-tests-'));
const outfile = path.join(tempDir, 'tax-invoice-reliability-regression.mjs');
...
const resolveSourcePath = async (sourcePath) => {
  const candidates = [sourcePath, `${sourcePath}.ts`, `${sourcePath}.tsx`, `${sourcePath}.js`];
...
try {
  ...
  await build({
    entryPoints: [path.join(projectRoot, 'tests/tax-invoice-reliability-regression.ts')],
    outfile,
    bundle: true,
    platform: 'node',
    format: 'esm',

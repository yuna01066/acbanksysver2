...
async (page) => {
  ...
  const pdfButton = page.getByRole('button', { name: '월별 PDF / 인쇄', exact: true });
  await pdfButton.waitFor();
  await page.getByRole('combobox', { name: 'PDF 출력 대상', exact: true }).click();
  await page.getByRole('option', { name: '테스트 직원 · 제작', exact: true }).click();
  const popupPromise = page.waitForEvent('popup');
  await pdfButton.click();
  const popup = await popupPromise;
  await popup.getByRole('region', { name: '직원별 일자 기록', exact: true }).waitFor();
  check(await popup.locator('.employee').count() === 1, 'PDF contains only the selected employee');
  check(await popup.evaluate(() => window.__printCalls === 1 && window.opener === null), 'PDF opens native printing and severs its opener');
  check(!(await popup.locator('body').innerText()).includes('테스트 예정 휴가'), 'PDF omits private leave reasons');
  await popup.close();
  await page.evaluate(() => { window.__qaOpen = window.open; window.open = () => null; });
  await pdfButton.click();
  await page.getByRole('alert').filter({ hasText: '팝업이 차단되었습니다' }).waitFor();
  check(await pdfButton.isEnabled(), 'blocked popup reports error and permits retry');
  await page.evaluate(() => { window.open = window.__qaOpen; });
  await overflow('mobile PDF toolbar has no body overflow');
  await page.screenshot({ path: 'output/playwright/monthly-report-mobile.png', fullPage: true });
  for (const table of ['attendance_records', 'profile_directory', 'leave_requests', 'company_holidays']) {
    await go('/attendance?role=admin&scope=all&tab=reports&fail=' + table);
    await page.getByRole('alert').filter({ hasText: '월별 근태 리포트를 불러오지 못했습니다' }).waitFor();
    check(await pdfButton.count() === 0, 'failed ' + table + ' query blocks PDF export');
  }
  await go('/attendance?role=employee&scope=all&tab=reports');
  await page.getByRole('button', { name: '퇴근하기', exact: true }).waitFor();
  check(await pdfButton.count() === 0, 'employee cannot access staff PDF through a forged URL');
  await go('/attendance?role=moderator&scope=all&tab=reports');
  await pdfButton.waitFor();
  check(await pdfButton.isEnabled(), 'moderator may export monthly attendance');
  check(await page.evaluate(() => window.__hubWrites.length === 0), 'PDF checks do not mutate even fixture records');
  await go('/attendance?role=admin&scope=all&tab=reports');
  await pdfButton.waitFor();

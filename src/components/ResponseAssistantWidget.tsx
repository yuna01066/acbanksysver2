...
const scenarios: Scenario[] = [
  {
    ...
    context: '이번 견적은 [요청 사양]과 [제작 방식], [공정 조건] 기준으로 산출된 내용입니다. 사양 변경 시 조정 가능 여부를 함께 검토할 수 있습니다.',
...
function buildDraft(message: string, context: string, tone: Tone) {
  ...
  if (tone === 'concise') {
    return [
      ...
      '문의 주신 내용 확인했습니다.',
      isPrice
        ? '이번 견적은 평균 단가라기보다 [요청 사양]과 [제작 방식] 기준으로 산출된 건으로 봐주시면 좋겠습니다.'
        : isSchedule
          ? '납기는 [제작 사양]과 [발주/결제 진행 시점] 확인 후 정확히 안내드릴 수 있습니다.'
  ...
  if (tone === 'soft') {
    return [
      ...
      '',
      isPrice
        ? '다만 이번 견적은 일반적인 평균 단가라기보다는 요청 주신 사양과 제작 방식에 따라 산출된 건으로 봐주시면 좋겠습니다.'
        : isSchedule
  ...
  return [
    ...
    '',
    isPrice
      ? '다만 이번 견적은 시장의 평균 단가로 단순 비교하기보다는, 요청 주신 사양과 제작 방식 기준으로 산출된 건으로 봐주시면 좋겠습니다.'
      : isSchedule
...
function reviewOutgoingText(text: string) {
  ...
  const replacements: Array<[RegExp, string, string]> = [
    ...
    [/이게 평균 단가입니다/g, '요청 주신 사양과 제작 방식에 따라 산출된 견적입니다', '평균 단가로 확정하면 고객의 가격 프레임을 그대로 받아들이게 됩니다.'],
    [/그 단가가 맞습니다/g, '현재 안내드린 견적은 요청 사양 기준으로 산출된 내용입니다', '단가를 방어하는 표현보다 산출 기준을 설명하는 쪽이 안전합니다.'],
    [/아크릴공장이라 아크릴만 합니다/g, '아크릴 기반으로 구현 가능한 방향을 먼저 검토해 보겠습니다', '회사 포지션을 단순 공장으로 좁히는 표현을 피합니다.'],
    [/판공장(?:입니다|이라서)?/g, '아크릴 소재와 제작 방식을 함께 검토하는 기준으로', '단순 판재/공장 이미지로 보이는 표현을 피합니다.'],
    [/인력이 없습니다/g, '현재 제작 일정상 인력이 모자라 일정 조율이 필요합니다', '역량 부재보다 일정 조율 문제로 표현합니다.'],
  ];
  ...
  if (/비싸|단가|가격|견적/.test(reviewed) && !/사양|산정|기준|공정|제작 방식/.test(reviewed)) {
    suggestions.push('가격 관련 답변에는 단순 방어보다 요청 사양, 제작 방식, 공정 조건 같은 설명 가능한 기준을 함께 넣는 것이 좋습니다.');
  }

  if (/마진|원가|견적 공식|계산식|상승률|몇 배|3배|20%/.test(reviewed)) {
    suggestions.push('내부 산출표, 원가, 마진, 상승률, 공정별 배수는 고객 문안에서 제외하고 [담당자 확인 필요]로 남기는 것이 안전합니다.');
  }

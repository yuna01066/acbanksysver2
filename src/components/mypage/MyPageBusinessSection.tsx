...
const MyPageBusinessSection: React.FC = () => {
  const { user } = useAuth();
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={() => navigate(`/saved-quotes/${quote.id}`)}>보기</Button>
                            <Button variant="destructive" size="sm" onClick={() => setDeleteQuoteId(quote.id)}>
        ...
        <TabsContent value="recipients" className="space-y-4">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold mb-2">
                      {syncValidationResult.invalid > 0 ? 'Pluuug 동기화 상태 변경 감지' : 'Pluuug 동기화 상태 정상'}
                    </h4>
                      ...
                      {syncValidationResult.invalid > 0 && (
                        <div>
                          ...
                          <ul className="mt-2 text-xs list-disc list-inside">
                            {syncValidationResult.clearedRecipients.map((name, idx) => <li key={idx}>{name}</li>)}
                          </ul>
                        </div>
                      )}
                    </div>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => setSyncValidationResult(null)}>닫기</Button>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="flex items-center gap-2"><Users className="h-5 w-5" />수신 담당자 리스트</CardTitle>
                <CardDescription>견적서에 등록된 수신 담당자 목록입니다.</CardDescription>
              </div>
              <div className="flex gap-2 flex-wrap justify-end">
                {getSyncedRecipients().length > 0 && (
                  <Button variant="outline" onClick={handleValidatePluuugSyncStatus} disabled={validatingSyncStatus || pluuugLoading} className="flex items-center gap-2">
                    {validatingSyncStatus ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                    {validatingSyncStatus ? '검증 중...' : '동기화 상태 검증'}
                  </Button>
                )}
                {recipients.length === 0 && (
                  <Button variant="outline" onClick={handleMigrateRecipients} disabled={migrating || recipientsLoading} className="flex items-center gap-2">
                ...
                {getRecipientsWithQuoteCounts().filter(r => !isRecipientSyncedToPluuug(r)).length > 0 && (
                  <Button onClick={handleBulkSyncAllToPluuug} disabled={bulkSyncing || pluuugLoading} className="flex items-center gap-2">
                    {bulkSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                    {bulkSyncing ? '동기화 중...' : '일괄 Pluuug 등록'}
                  </Button>
                )}
                <Button variant="outline" onClick={handleImportFromPluuug} disabled={importingFromPluuug || pluuugLoading} className="flex items-center gap-2">
                  {importingFromPluuug ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
            ...
            <CardContent>
              {importResult && (
                <Card className="mb-4 border-blue-500/50 bg-blue-50/50 dark:bg-blue-900/10">
                  <CardContent className="pt-4 pb-3">
                    <div className="flex items-start gap-3">
                      <CheckCircle2 className="h-5 w-5 text-blue-600 flex-shrink-0 mt-0.5" />
                      <div className="flex-1 text-sm">
                        <p className="font-medium mb-1">Pluuug 가져오기 완료</p>
                        <p className="text-muted-foreground">새로 가져온 고객: {importResult.imported}명 / 이미 등록됨: {importResult.skipped}명</p>
                        {importResult.importedClients.length > 0 && (
                          <ul className="mt-1 text-xs list-disc list-inside text-muted-foreground">
                            {importResult.importedClients.map((name, idx) => <li key={idx}>{name}</li>)}
                          </ul>
                        )}
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => setImportResult(null)}>닫기</Button>
                    </div>
                  </CardContent>
                </Card>
              )}
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="relative flex-1">
                      ...
                      <Input placeholder="회사명, 담당자, 연락처 검색..." value={recipientSearch} onChange={e => setRecipientSearch(e.target.value)} className="pl-9 pr-8" />
                      {recipientSearch && (
                        <Button variant="ghost" size="sm" onClick={() => setRecipientSearch('')} className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7 p-0">
                          <X className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Select value={recipientSyncFilter} onValueChange={(v: 'all' | 'synced' | 'unsynced') => setRecipientSyncFilter(v)}>
                        <SelectTrigger className="w-[160px]"><SelectValue placeholder="동기화 상태" /></SelectTrigger>
                        <SelectContent>
                  ...
                  {(recipientSearch || recipientSyncFilter !== 'all') && (
                    <div className="text-sm text-muted-foreground">
                      ...
                      {recipientSearch && <span> (검색어: "{recipientSearch}")</span>}
                      {recipientSyncFilter !== 'all' && <span> ({recipientSyncFilter === 'synced' ? 'Pluuug 연동됨' : '미연동'})</span>}
                    </div>
                  )}
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {getFilteredRecipients().length === 0 ? (
                          <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">검색 결과가 없습니다.</TableCell></TableRow>
                        ) : (
                          getFilteredRecipients().map((recipient) => {
                            const synced = isRecipientSyncedToPluuug(recipient);
                            const key = `${recipient.company_name}-${recipient.contact_person}`;
                            return (
                              <TableRow key={recipient.id} className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => setSelectedRecipient(recipient)}>
                                ...
                                <TableCell>
                                  {synced ? (
                                    <Badge variant="secondary" className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"><Cloud className="w-3 h-3 mr-1" />연동됨</Badge>
                                  ) : (
                                    <Button size="sm" variant="outline" onClick={e => handleSyncRecipientToPluuug(recipient, e)} disabled={syncingRecipient === key || pluuugLoading} className="text-xs">
                                      {syncingRecipient === key ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <Upload className="w-3 h-3 mr-1" />}Pluuug 등록
                                    </Button>
                                  )}
                                </TableCell>
                                <TableCell className="text-right"><span className="font-semibold text-primary">{recipient.quoteCount}건</span></TableCell>
                                <TableCell className="text-center">
                                  <div className="flex items-center justify-center gap-1">
                                    <Button size="sm" variant="ghost" onClick={e => { e.stopPropagation(); setEditingRecipient(recipient); }} className="h-8 w-8 p-0" title="수정">
                                      <Pencil className="w-4 h-4" />
                                    </Button>
                                    <Button size="sm" variant="ghost" onClick={e => { e.stopPropagation(); setDeleteRecipientTarget(recipient); }} className="h-8 w-8 p-0 text-destructive hover:text-destructive hover:bg-destructive/10" title="삭제">
...
export default MyPageBusinessSection;

...
function addError(result: MigrationResult, message: string) {
...
serve(async (req) => {
  ...
  try {
    ...
    // SAFETY: dryRun defaults to TRUE. It must be explicitly set to false to copy files.
    const dryRun = body.dryRun !== false;
    const updateDatabase = body.updateDatabase !== false;
    // SAFETY: Source files in Supabase Storage are NEVER deleted by this function.
    const sourceDeleted = false;
    const bucketsToMigrate = Array.isArray(body.buckets) && body.buckets.length
      ? body.buckets
      : DEFAULT_BUCKETS;
    ...
    for (const bucketName of bucketsToMigrate) {
      ...
      const listRecursive = async (prefix: string) => {
        ...
        if (error) {
          addError(results[bucketName], `List ${prefix || '/'}: ${error.message}`);
          return;
        }
        if (!data) return;
      ...
      await listRecursive('');
      results[bucketName].scanned = allFiles.length;
      results[bucketName].planned = allFiles.length;
      results[bucketName].totalBytes = allFiles.reduce((sum, file) => sum + file.size, 0);
      results[bucketName].samples = allFiles.slice(0, 10).map((file) => `${bucketName}/${file.path}`);
      ...
      for (const file of allFiles) {
        ...
        try {
          ...
          if (downloadError || !fileData) {
            addError(results[bucketName], `Download ${filePath}: ${downloadError?.message}`);
          ...
          addError(results[bucketName], `${filePath}: ${err instanceof Error ? err.message : 'unknown'}`);
          results[bucketName].failed++;
        }
      }
    }
    ...
    const databaseUpdates = {
      documentFiles: 0,
      taxDocuments: 0,
      internalProjectDocuments: 0,
      recipients: 0,
      teamMessages: 0,
      incidentReports: 0,
    };

    if (!dryRun && updateDatabase) {
      for (const bucketName of bucketsToMigrate) {
        ...
        for (const row of ledgerRows || []) {
          ...
          await supabase
            ...
            .update({
              ...
              storage_path: `${bucketName}/${row.storage_path}`,
            } as any)
            .eq('id', row.id);
          databaseUpdates.documentFiles++;
        }
      }
      ...
      for (const doc of internalDocs || []) {
        if (doc.file_url && !doc.file_url.startsWith('internal-project')) {
          ...
          databaseUpdates.internalProjectDocuments++;
        }
      }
      ...
      for (const inc of incidents || []) {
        if (inc.attachments && Array.isArray(inc.attachments)) {
          ...
          if (changed) {
            ...
            databaseUpdates.incidentReports++;
          }
        }
      }
    }
    ...
    return new Response(JSON.stringify({
      ...
      sourceDeleted,
      message: dryRun
        ? 'Dry-run only. No files were copied and no database records were changed.'
        : 'Files were copied to GCS. Supabase Storage originals were not deleted.',
      results,
      databaseUpdates,
    }), {

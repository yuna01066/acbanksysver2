-- Prevent duplicate daily attendance records per employee.
-- Existing duplicates are backed up, merged into one record per user/date, then guarded by a unique index.

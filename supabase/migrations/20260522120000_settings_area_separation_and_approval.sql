-- Split sensitive company settings from operational admin settings and add approval workflow.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'page_access_permissions_effect_check'
  ) THEN
    ALTER TABLE public.page_access_permissions
      ADD CONSTRAINT page_access_permissions_effect_check
...
AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = auth.uid()
      AND lower(p.email) = 'acbank@acbank.co.kr'
  );
$$;
...
BEGIN
  ...
  IF NEW.requested_by_name IS NULL THEN
    SELECT p.full_name INTO NEW.requested_by_name
    FROM public.profiles p
...
FOR EACH ROW
EXECUTE FUNCTION public.fill_settings_change_request_requester();
...
ON public.settings_change_requests
FOR ALL
TO authenticated
USING (public.has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));
...
BEGIN
  ...
  SELECT pap.effect INTO user_effect
  FROM public.page_access_permissions pap
  WHERE pap.page_key = _feature_key
    AND pap.user_id = auth.uid()
  LIMIT 1;
...
BEGIN
  IF _request.target_table = 'response_assistant_settings' THEN
    IF _request.action = 'delete' THEN
      DELETE FROM public.response_assistant_settings
      WHERE key = _request.target_key;
      RETURN;
    END IF;
    ...
    VALUES (
      _request.target_key,
      COALESCE(_request.after_value->>'value', ''),
      _request.after_value->>'description',
      auth.uid()
    )
  ...
  IF _request.target_table = 'page_role_access' THEN
    ...
    VALUES (
      _request.target_key,
      COALESCE(_request.after_value->>'min_role', 'admin')
    )
...
AS $$
DECLARE
  request_row public.settings_change_requests;
BEGIN
  ...
  FROM public.settings_change_requests
  WHERE id = _request_id
  FOR UPDATE;

  IF request_row.id IS NULL THEN
    RAISE EXCEPTION 'settings change request not found';
  END IF;

  IF request_row.status <> 'pending' THEN
    RAISE EXCEPTION 'settings change request is not pending';
  END IF;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'pending settings change request not found';
  END IF;

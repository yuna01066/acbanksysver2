...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'portfolio_posts_gallery_type_check') THEN
    ALTER TABLE public.portfolio_posts
...
CREATE INDEX IF NOT EXISTS idx_portfolio_posts_gallery_type_created_at
  ...
  WHERE archived_at IS NULL AND COALESCE(visibility, 'published') = 'published';
...
RETURNS TABLE (
  id UUID, title TEXT, gallery_type TEXT, memo TEXT, category TEXT, client_name TEXT,
  project_year INTEGER, location TEXT, materials TEXT[], processes TEXT[], visibility TEXT,
  archived_at TIMESTAMPTZ, cover_image_id UUID, keywords TEXT[], created_by TEXT,
  created_at TIMESTAMPTZ, updated_at TIMESTAMPTZ, image_count BIGINT, total_count BIGINT
)
LANGUAGE sql SECURITY INVOKER STABLE SET search_path = public
AS $$
  WITH normalized AS (
    SELECT NULLIF(BTRIM(p_search_text), '') AS search_text,
      COALESCE(p_category_keywords, '{}'::TEXT[]) AS category_keywords,
      CASE WHEN p_gallery_type IN ('portfolio','reference') THEN p_gallery_type ELSE 'portfolio' END AS gallery_type,
  ...
  matched AS (
    ...
    WHERE COALESCE(p.gallery_type, 'portfolio') = n.gallery_type
  ...
  SELECT matched.id, matched.title, COALESCE(matched.gallery_type,'portfolio') AS gallery_type,
    matched.memo, matched.category, matched.client_name, matched.project_year, matched.location,

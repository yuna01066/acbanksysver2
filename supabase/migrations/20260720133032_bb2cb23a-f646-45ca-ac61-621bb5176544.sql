...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'public_booking_requests_consultation_lead_id_fkey') THEN
    ALTER TABLE public.public_booking_requests
      ADD CONSTRAINT public_booking_requests_consultation_lead_id_fkey
      FOREIGN KEY (consultation_lead_id) REFERENCES public.client_consultation_leads(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'public_booking_requests_assigned_to_fkey') THEN
    ALTER TABLE public.public_booking_requests
      ADD CONSTRAINT public_booking_requests_assigned_to_fkey
      FOREIGN KEY (assigned_to) REFERENCES public.profiles(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'public_booking_requests_meeting_mode_check') THEN
    ALTER TABLE public.public_booking_requests
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'client_consultation_leads_public_booking_request_id_fkey') THEN
    ALTER TABLE public.client_consultation_leads
      ADD CONSTRAINT client_consultation_leads_public_booking_request_id_fkey
      FOREIGN KEY (public_booking_request_id) REFERENCES public.public_booking_requests(id) ON DELETE SET NULL;
  END IF;
END $$;
...
BEGIN
  ...
  SELECT name INTO conflict_name FROM busy LIMIT 1;

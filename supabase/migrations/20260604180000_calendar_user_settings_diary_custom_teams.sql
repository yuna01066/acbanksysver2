-- Add persisted calendar preferences, private diary entries, and custom team calendars.
...
CREATE INDEX IF NOT EXISTS idx_calendar_diary_entries_owner_date
  ON public.calendar_diary_entries(owner_id, diary_date);

CREATE INDEX IF NOT EXISTS idx_calendar_teams_active_name
  ON public.calendar_teams(is_active, name);

CREATE INDEX IF NOT EXISTS idx_calendar_team_members_user
  ON public.calendar_team_members(user_id, team_id);
...
ON public.calendar_diary_entries
FOR ALL
USING (auth.uid() = owner_id)
WITH CHECK (auth.uid() = owner_id);

DROP POLICY IF EXISTS "Authenticated users can view active calendar teams" ON public.calendar_teams;
CREATE POLICY "Authenticated users can view active calendar teams"
ON public.calendar_teams
FOR SELECT
...
CREATE POLICY "Admins can manage calendar teams"
ON public.calendar_teams
FOR ALL
USING (
...
CREATE POLICY "Authenticated users can view calendar team members"
ON public.calendar_team_members
FOR SELECT
USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Admins can manage calendar team members" ON public.calendar_team_members;
CREATE POLICY "Admins can manage calendar team members"
ON public.calendar_team_members
FOR ALL
USING (
  public.has_role(auth.uid(), 'admin'::public.app_role)
...
CREATE TRIGGER update_calendar_diary_entries_updated_at
BEFORE UPDATE ON public.calendar_diary_entries
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_calendar_teams_updated_at ON public.calendar_teams;
CREATE TRIGGER update_calendar_teams_updated_at
BEFORE UPDATE ON public.calendar_teams
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

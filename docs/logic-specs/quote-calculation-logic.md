...
2. 원판 단가 결정
   ...
   - 2026-06-01 이후 `glossy-color` 단가는 A/B 공급사 중 높은 단가에 3% 버퍼를 적용한다. B `정3X6`은 내부 `대3*6`, B `3X6(소)`는 내부 `소3*6`로 매핑하며, 신규 견적 계산기 선택지는 `소3*6`/`대3*6` 두 구간만 유지한다. 기존 `3*6` row는 과거 견적 호환용 레거시 데이터로 비활성 보존한다.
   - 사틴은 `대3*6`, `1*2`, `4*8`만, 아스텔은 `대3*6`, `4*5`, `대4*5`, `1*2`, `4*8`만 생산 가능 규격으로 선택한다.
